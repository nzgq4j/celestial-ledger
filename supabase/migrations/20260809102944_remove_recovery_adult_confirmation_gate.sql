-- Recovery plan:
-- Reapply the preceding queue_paid_report definition from
-- 20260807151036_consume_report_entitlement_on_completion.sql if the
-- per-report age gate must be restored. This migration changes no stored data.
...
begin
  ...
  -- A report reserves its entitlement through the unique entitlement_id
  -- relationship. Return that job for repeat requests, but do not decrement
  -- the user's visible allowance until complete_report_job succeeds.
...
comment on function public.queue_paid_report(uuid,uuid,uuid,text,text,text,text,jsonb) is
  'Queues one owner-authorized report per entitlement without a per-report age confirmation; consumption occurs only after successful completion.';
